...
async function runToggle (context, issue, sha, deps, owner) {
  ...
  if (deps.length > 0) {
    ...
    for (const number of deps) {
      ...
      log("Issue Data2", { owner, ...issue })
      const dependentIssue = await github.issues.get({ owner, ...issue, number })
      log("Received dependentIssue data")
      // The actual test
      if (dependentIssue.data.state === 'open') {
        // Add the label
        log('Adding label')
        await github.issues.addLabels({ owner, ...issue, labels: ['Dependent'] })
        foundOpenIssue = true
        break
      }
    }
  ...
  if (deps.length == 0 || foundOpenIssue == false) {
    try {
      ...
      await github.issues.removeLabel({ owner, ...issue, name: 'Dependent' })
    } catch (err) {
